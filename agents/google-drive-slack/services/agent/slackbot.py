...
class SlackBot:
    ...
    async def reply_to_slack(self, channel_id, user_id ,message, thread_ts=None, message_ts=None):
        ...
        if re.match(slack_emoji_regex, message.strip()):
            try:
                emoji_name=message.strip().replace(":", "")
            ...
            if thread_ts==None or message_ts==None:
               print("thread is empty")
               mes= "@"+user_id + message
               return await self.client.chat_postMessage(channel=channel_id, text= mes)

            else:
                
                
                return await self.client.chat_postMessage(channel=channel_id, text=message, thread_ts=thread_ts)
    ...
    async def respond_to_message(self, channel_id, thread_ts, message_ts, user_id, text):
        try:
            slack_agent: SlackAgent = self.threads_bot_is_participating_in.get(thread_ts, None)
            ...
            chat_history= await self.get_thread_history(message_ts,thread_ts,channel_id)
            print("chat_history", chat_history)
            response = await slack_agent.respond(sender_user_info, text, chat_history, channel_id)
            ...
            # elif response.find("off topic")!=1:
            #     await self.reply_to_slack(channel_id, user_id, response, thread_ts, message_ts)
    ...
    async def add_ai_to_thread(self, channel_id, thread_ts):
        if thread_ts in self.threads_bot_is_participating_in:
            return

        ai= SlackAgent(self.bot_user_name,self.client)

        self.threads_bot_is_participating_in[thread_ts] = ai

        return ai
    
    async def get_thread_history(self,message_ts,thread_ts, channel_id):
        processed_history = []
        ...
        if not self.is_parent_thread_message(message_ts, thread_ts):
            ...
            for message in message_history:
                ...
                print("message user" , user_id)
                user_name = await self.get_username_for_user_id(user_id)
                print("message username" , text)
                if (user_id == self.bot_user_id):
                    processed_history.append(AIMessage(content=text))
                else:
                    # Get the username for this user_id:
                    processed_history.append(HumanMessage(content= user_name + text))
    ...
    async def on_message(self, event, say):
        ...
        self.bot_user_id= os.environ.get("SLACK_BOT_USER_ID")
        try:
...
"""])).generations[0][0].text
        if welcome_message:
          try:
            ...
            logger.exception("Error sending welcome message", e)
