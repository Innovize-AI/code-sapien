#sales research agent


# to add
Best time to reach

Add more prospect findings such as Hiring for a position
Likely to convert

Auto add leads scraping linkedin (to check if possible)


Past Interactions via email, slack etc and next recommendations




# Improve system 

Proposal creator
Meeting notes generator
Sales Assistant to chat and get recommendations
 
CSV upload of prospects (linkedin url and websites)

# add goal to outreach

# integrate inbox manager with salesresearch

# add predicive analytics to see the lead score and next best action

# get past conversation history from CRM or emails - Done

# get reactions to posts and comments to see if the lead is interested in the product/service

# extract website from linkedin profile or viceversa

# focus on report itself (use tavily deep agents or langraph depp agents)

# waterfall likeoutreach priority (comment, hiring, etc)

# add competitor lead gen